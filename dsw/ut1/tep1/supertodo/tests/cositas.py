...
def test_add_task_with_get_method(client: Client):
    ...
    assert 'complete_before' in response.context['form'].fields
...
def test_edit_task_with_get_method(client: Client, task):
    ...
    assert 'description' in response.context['form'].fields
    assert 'complete_before' in response.context['form'].fields
    assert 'done' in response.context['form'].fields
    assert 'slug' not in response.context['form'].fields
...
def test_unique_constraints_for_task(client: Client, task):
    with pytest.raises(IntegrityError):
        baker.make(Task, slug=task.slug)


@pytest.mark.django_db
def test_default_values_for_task_fields():
    task = baker.make(Task)
    assert task.description == ''
    assert task.done is False
    assert task.complete_before is None
